...
def main():
    ...
    for index, row in dataframe_from_inj.iterrows():
        ...
        inclination = row["inclination_EM"] * 360.0 / (2 * np.pi)
        ...
        if args.telescope.lower() == "ultrasat":
            command = f"gwemopt-run --telescopes {args.telescope} --geometry 3d --doTiles --doSchedule  --scheduleType greedy_slew --doTrueLocation --inclination --true_location  --true_ra {ra} --true_dec {dec} --true_distance {dist} --true_inclination {inclination} --powerlaw_cl 0.9 --doObservability --doObservabilityExit --timeallocationType powerlaw   --gpstime {gpstime} --event {skymap_file} --filters {args.filters} --exposuretimes {exposuretime} --doSingleExposure --doAlternatingFilters --doEfficiency --lightcurveFiles {lc_file}  -o {outdir} --modelType file --doPlots --doUsePrimary  --ignore_observability"
