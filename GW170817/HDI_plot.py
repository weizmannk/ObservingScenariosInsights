...
def plot_hist(ax, data, color, **kwargs):
    """
    Custom function to plot histogram with HDI on the axes provided.
    """
    # Histogram with HDI
    density, bins, _ = ax.hist(
        data, color=color, histtype="stepfilled", alpha=0.3, density=True, **kwargs
    )
    hdi = az.hdi(data, hdi_prob=0.95)  # 95% HDI
    ax.axvline(x=hdi[0], color="k", linestyle="--")
    ax.axvline(x=hdi[1], color="k", linestyle="--")
    mid = 0.95 * max(density)
    ax.text(hdi[0], mid, f"{hdi[0]:.2f}", horizontalalignment="right", color="k")
    ax.text(hdi[1], mid, f"{hdi[1]:.2f}", horizontalalignment="left", color="k")
    ...
    **kwargs
    ...
    if len(data) == 2:
        ...
        for i in range(data[0].shape[1]):
            qs1 = corner.quantile(data[0].T[i], [0.05, 0.5, 0.95])
            qs2 = corner.quantile(data[1].T[i], [0.05, 0.5, 0.95])
